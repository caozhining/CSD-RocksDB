ops_path="./CPU_ARRAY_4t_ops.csv"
echo $ops_path
report_path="./CPU_ARRAY_4t_top.txt"
echo $report_path
/home/zhining/rocksdb-9.0.0-ARRAY/db_bench --db=/mnt/smartssd3/czn/rocksdb_array_cpu --benchmarks="fillrandom,stats" --num=1000000000 --compression_type=None --key_size=24 --value_size=1024 --format_version=5 --max_background_compactions=4 --max_background_jobs=5 --target_file_size_multiplier=1 --subcompactions=1 --max_background_flushes=-1 --stats_dump_period_sec=120 --open_files=500 --report_interval_seconds=1 --report_file=$ops_path &
while true; do
    top -b -n 1 | grep "db_bench" >> $report_path
    sleep 1
done