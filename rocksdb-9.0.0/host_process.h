...
public:
    ...
    {
        // set properties
        num_data_blocks = pps[CSD_PPS_DATABLOCK_NUM_OFF];
        num_entries = pps[CSD_PPS_ENTRIES_OFF];
        // std::cout<<"pps1:" << pps[CSD_PPS_ENTRIES_OFF] << "num_entries:" << num_entries << '\n'; 
        data_size = pps[CSD_PPS_DATASIZE_OFF];
        raw_key_size = pps[CSD_PPS_RAWKEYSIZE_OFF];
        raw_value_size = pps[CSD_PPS_RAWVALUESIZE_OFF];
        tail_start_offset = pps[CSD_PPS_INDEXBLOCK_OFFSET_OFF];
        // init data block
