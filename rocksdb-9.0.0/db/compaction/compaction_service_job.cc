...
CompactionJob::ProcessKeyValueCompactionOnCSD(
  ...
  cl_int err;
  uint32_t now_acc_id = compaction->GetCompactionAccleratorId();

  std::cout<<"now acc id "<<now_acc_id<<"\n";

  cl::Context context = cfd->Compaction_csd_context[now_acc_id];
  cl::CommandQueue q = cfd->Compaction_csd_queue[now_acc_id];
  cl::Kernel Compaction_kernel = cfd->Compaction_accelerator_kernel[now_acc_id];
  ...
  for (const auto& files_per_level : inputs) {
    for (const auto& file : files_per_level.files) {
      ...
      ROCKS_LOG_INFO(
        db_options_.info_log,
        "[%s] [JOB %d] The input file number for Compaction on CSD is %lu, smallest key %s, largest key %s. ",
        sub_compact->compaction->column_family_data()->GetName().c_str(), job_id_,
        input_fd_num, file->smallest.DebugString(true).c_str(), file->largest.DebugString(true).c_str()
        );
  ...
  if(compaction->immutable_options()->compaction_csd_gen_sst_file_size_policy != kCompactionCSDSSTavg)
  {
    ...
    {
      uint64_t tune_size=62*1024*1024*4*(1<<(compaction->output_level()-1));
      if(tune_size>=400*1024*1024*4)
      {
        tune_size = 400*1024*1024*4;
      }
      if(gen_sst_file_limit<tune_size)
      {
  ...
  uint64_t total_used_memory_space = (gen_sst_file_limit + 8*15 + (450 * 4 * 1024 *1024) + (8 * 1024 *1024) + 4*(128*4 + 20));
  uint64_t memory_total_space = 4*1024*1024;
  memory_total_space *= 1024;
  printf("gen_sst_file_limit %llu\n", gen_sst_file_limit);
  printf("totol used memory space %llu\n", total_used_memory_space);
  std::cout<<(total_used_memory_space<=memory_total_space)<<"\n";
  assert(total_used_memory_space<=memory_total_space);
  ...
  cl::Buffer host_data   = cfd->input_metadata_buffer[now_acc_id];
  cl::Buffer sst_buffer  = cfd->output_datablock_buffer[now_acc_id];
  cl::Buffer index_block_buffer = cfd->output_indexblock_buffer[now_acc_id];
  cl::Buffer output_data = cfd->output_metadata_buffer[now_acc_id];
  ...
  for(int x=0; x<file_count; x++)
  {
      ...
      close(fd[x]);
      std::cout<<"sst read : "<< total_read_size <<"\n";
  }
  ...
  std::vector<uint32_t> output_path_id_meta = compaction->output_path_id_meta();
  uint32_t output_level = compaction->output_level();

  uint32_t output_path_id = 0;
  for(int i=0;i<output_path_id_meta.size();i++){
    if(output_path_id_meta[i]>0){
      output_path_id = i;
    }
  }
  ...
  for (int i = 0; i < CSD_MAX_OUTPUT_FILE_NUM; i++)
  {
    ...
    if (entries <= 0){
        ...
        printf("Empty file, sst_length is %ld, index_length %ld, entries %ld\n", sstlength, indexlength, entries);
        continue;
    }
    if (sstlength <= 0){
      std::cout << "output file num less than 4: " << i + 1 << "\n";
      break;
    }
    ...
    if(output_level==1){
      output_path_id = (output_path_id+1)%(4);
    }else{
      while(output_path_id_meta[output_path_id]==0){
        output_path_id++;
      }
      output_path_id_meta[output_path_id]--;
    } 
    // output_path_id = 0;
    // std::cout<<"CSD output path id = "<<output_path_id<<"\n";
    ...
    printf("CSD write SST id: %ld, path %d, sst_length is %ld, index_length %ld, entries %ld\n", file_id, output_path_id, sstlength, indexlength, entries);
    ...
    // 启用 Direct I/O，跳过 OS Page Cache
    env_options.use_direct_reads = true;
    env_options.use_direct_writes = true;

    // 使用 O_DIRECT 时，一般建议自己控制 buffer 大小和对齐。
    // WritableFile 内部会有 buffer，设置一个合理的缓冲区大小（比如4MB）
    env_options.writable_file_max_buffer_size = 4 * 1024 * 1024;  // 4MB buffer

    // 控制多久进行一次 fdatasync（磁盘同步），保证数据可靠性
    env_options.bytes_per_sync = 4 * 1024 * 1024;  // 写4MB后自动sync一次

    // 控制 compact 时读取时的 readahead（加快 compact）
    env_options.compaction_readahead_size = 2 * 1024 * 1024;  // 2MB

    // 文件描述符设置，不需要特殊改动，默认就行
    env_options.set_fd_cloexec = true;

    // 通常不启用 mmap，因为 Direct I/O 和 mmap 是互斥的
    env_options.use_mmap_reads = false;
    env_options.use_mmap_writes = false;

    env_options.rate_limiter = cfd->ioptions()->rate_limiter.get();

    rocksdb::Status s = env_->NewWritableFile(tgt_file, &file, env_options);
    if (!s.ok()){
      std::cout << "文件创建失败: " << s.ToString() << std::endl;
      assert(0);
    }else{
    ...
    uint64_t index_block_write_size;
    uint64_t meta_data_offset;

    // 两种实现方法
    // 第一种： 直接将properties metaindexblock和footer写到index block buffer当中，然后通过slice2写下去
    ...
    if(now_write_plan==1){ 
      ...
      char data_block_padding_buffer[4096];

      for(int x=0; x<4096; x++){
        data_block_padding_buffer[x] = 0;
      }
      memcpy((sst_result + (offset_sst[i])+ sstlength), data_block_padding_buffer, index_block_offset-sstlength);


      uint64_t otherlength = pp_index + 5 + metaindexblock_index + 5 + 53;
      char host_buffer[CSD_ALIGN_TO_4K(otherlength)+4096*2];

      char* write_buffer = host_buffer + (4096-reinterpret_cast<size_t>(host_buffer) % 4096);
      //Copy properties metaindex block and footer_buffer to write_buffer.
      memcpy((write_buffer),                                            properties,     pp_index + 5);
      memcpy((write_buffer) + pp_index + 5,                             metaindexblock, metaindexblock_index + 5);
      memcpy((write_buffer) + pp_index + 5 + metaindexblock_index + 5,  footer_buffer,  53);

      memcpy((index_block_result+offset_index[i]+index_block_write_size), write_buffer, otherlength + ( CSD_ALIGN_TO_4K(index_block_write_size+otherlength) - (index_block_write_size+otherlength)));
      ...
      rocksdb::Slice slice1(sst_result + (offset_sst[i]), index_block_offset);
      rocksdb::Slice slice2(index_block_result + (offset_index[i]), index_block_write_size);

      int retry = 0;
      do {
        s = file->Append(slice1);
        if (!s.ok()) sleep(1);  // 失败就等1秒
      } while (!s.ok() && retry++ < 3); // 最多重试3次
      if (retry)
      {
        std::cout << "slice1触发retry = " << retry << std::endl;
      }
      
      if (s.ok()) {
        // std::cout << "slice1写入成功" << std::endl;
        retry = 0;
        do {
          s = file->Append(slice2);
          if (!s.ok()) sleep(1);  // 失败就等1秒
        } while (!s.ok() && retry++ < 3); // 最多重试3次
        if (retry)
        {
          std::cout << "slice2触发retry = " << retry << std::endl;
        }
      }
      if (s.ok())
      {
        // std::cout << "slice2写入成功" << std::endl;
        // std::cout << "write_size = " << file_size << std::endl;
        std::cout << "index_write_size = " << index_block_write_size << std::endl;
      }
      else
      {
        std::cout << "数据写入失败: " << s.ToString() << std::endl;
        std::cout << "Error acc num : "<< now_acc_id << "\n";
        std::cout << "write_size = " << file_size << std::endl;
        std::cout << "index_write_size = " << index_block_write_size << std::endl;
        assert(0);
      }
    ...
    if(now_write_plan == 2){
      index_block_offset = CSD_ALIGN_TO_4K(sstlength);
      index_block_write_size = CSD_ALIGN_TO_4K(indexlength);
      meta_data_offset = index_block_write_size + index_block_offset;
      pp_index = 0;
      metaindexblock_index = 0;
      ...
      uint64_t otherlength = pp_index + 5 + metaindexblock_index + 5 + 53;
      char* write_buffer = static_cast<char*>(std::aligned_alloc(4096, CSD_ALIGN_TO_4K(otherlength)));
      //Copy properties metaindex block and footer_buffer to write_buffer.
      memcpy((write_buffer),                                            properties,     pp_index + 5);
      memcpy((write_buffer) + pp_index + 5,                             metaindexblock, metaindexblock_index + 5);
      memcpy((write_buffer) + pp_index + 5 + metaindexblock_index + 5,  footer_buffer,  53);
      ...
      rocksdb::Slice slice1(sst_result + (offset_sst[i]), index_block_offset);
      rocksdb::Slice slice2(index_block_result + (offset_index[i]), index_block_write_size);
      rocksdb::Slice slice3(write_buffer, CSD_ALIGN_TO_4K(otherlength));
      ...
      if (s.ok()) {
        ...
        if(s.ok()) {
          std::cout << "slice2写入成功" << std::endl;
          do {
            s = file->Append(slice3);
            if (!s.ok()) sleep(1);  // 失败就等1秒
          } while (!s.ok() && retry++ < 3); // 最多重试3次
          if (retry){
            std::cout << "slice3触发retry = " << retry << std::endl;
          }
        }
      }
      if (s.ok())
      {
        std::cout << "slice3写入成功" << std::endl;
        std::cout << "write_size = " << file_size << std::endl;
        std::cout << "index_write_size = " << index_block_write_size << std::endl;
      }
      else
      {
        std::cout<<"sst_buffer address : "<< static_cast<void*>(sst_result)<< "\n";
        std::cout<<"sst_buffer offset : " << offset_sst[i] <<"\n";
        std::cout<<"index buffer address : "<< static_cast<void*>(index_block_result) <<"\n";
        std::cout<<"index buffer offset : " << offset_index[i] <<"\n";
        char* write_buffer = static_cast<char*>(std::aligned_alloc(4096, index_block_write_size));
        for(int x = 0; x<4096 ; x++){
          write_buffer[x] = '0';
        }
        rocksdb::Slice slicex(write_buffer, index_block_write_size);
        std::cout << "数据写入失败: " << s.ToString() << std::endl;
        s = file->Append(slicex);
        if(s.ok()){
          std::cout<<"数据写入成功, 不是文件的问题！\n";
        }else{
          std::cout << "数据写入失败，文件的问题！: " << s.ToString() << std::endl;
        }
        std::cout << "Error acc num : "<< now_acc_id << "\n";
        std::cout << "write_size = " << file_size << std::endl;
        std::cout << "data block write size = " << index_block_offset << "\n";
        std::cout << "index_write_size = " << index_block_write_size << std::endl;
        assert(0);
      }
    }
    ...
    ROCKS_LOG_INFO(
      db_options_.info_log,
      "[%s] [JOB %d] The output file number for Compaction on CSD is %lu, smallest key %s, largest key %s. ",
      sub_compact->compaction->column_family_data()->GetName().c_str(), job_id_,
      output_path_id, meta.smallest.DebugString(true).c_str(), meta.largest.DebugString(true).c_str()
      );
    ...
    s = GetSstInternalUniqueId(db_id_, db_session_id_, file_id,
                              &meta.unique_id);
  ...
  //   // char test[50] = {0};
  //   // int rdsz = pread(nvmeFd, test, 50, index_block_offset);
  //   // if (rdsz != 50)
  //   // {
  //   //   std::cout << "test read Error! read size = " << rdsz << std::endl;
  //   // }
  //   // int cnt = 0;
  //   // for (int i2 = 0; i2 < 50; i2++)
  //   // {
  //   //   printf("%02x ", (uint32_t)test[i2] & 0xFF);
  //   //   if (test[i2] == 0) cnt++;
  //   // }
  //   // printf("\n");
  //   // if (cnt == 50)
  //   // {
  //   //   std::cout << "test index block wrong" << std::endl;
  //   //   for (int i2 = 0; i2 < 50; i2++)
  //   //   {
  //   //     printf("%02x ", (uint32_t)(index_block_result + (offset_index[i]))[i2] & 0xFF);
  //   //   }
  //   // }
  //   // printf("\n");
  ...
  //   // uint64_t lpremain = index_block_offset % chunksize;
  //   // uint64_t lptms = index_block_offset / chunksize + (lpremain != 0);
  //   // std::cout << "extra sst write length: " << lpremain << " bytes\n";
  //   // for (uint64_t j = 0; j < lptms ; j++)
  //   // {
  //   //   if ((j == lptms-1) && lpremain)
  //   //   {
  //   //     write_size = pwrite(nvmeFd, sst_result + (offset_sst[i]) + chunksize * j, lpremain, j*chunksize);
  //   //     std::cout << "extra sst write : " << write_size << " bytes\n";
  //   //     check_pwrite(write_size);
  //   //   }
  //   //   else
  ...
  //   // lpremain = index_block_write_size % chunksize;
  //   // lptms = index_block_write_size / chunksize + (lpremain != 0);
  //   // std::cout << "extra index write length: " << lpremain << " bytes\n";
  //   // for (uint64_t j = 0; j < lptms ; j++)
  //   // {
  //   //   if ((j == lptms-1) && lpremain)
  //   //   {
  //   //     write_size = pwrite(nvmeFd, index_block_result + (offset_index[i]) + chunksize * j, lpremain, index_block_offset+j*chunksize);
  //   //     std::cout << "extra index write : " << write_size << " bytes\n";
  //   //     check_pwrite(write_size);
  //   //   }
  //   //   else
