...
// merge operator disabled by default due to link error
  ...
  opt.max_background_jobs = 5;
  opt.max_background_compactions = 4;
  opt.max_background_flushes = -1;
  opt.stats_dump_period_sec = 120;
  opt.max_subcompactions = 1;

  rocksdb::DBOptions db_opt(opt);
  ...
  // 设置其他列族参数
  cf_opt.target_file_size_multiplier = 1;
  cf_opt.compaction_device = rocksdb::kCompactionOnCSD;
  cf_opt.CompactionKernelPath="/home/yjr/projects/compaction_varkey/build_dir.hw.xilinx_u2_gen3x4_xdma_gc_2_202110_1/compaction_k32v1024_20250606.xclbin";
  cf_opt.Compaction_accelerator_id.resize(4);
  cf_opt.Compaction_accelerator_id= {1, 0, 2, 3};
  cf_opt.compaction_csd_gen_sst_file_size_policy = rocksdb::KCompactionCSDSSTlayer;
  uint64_t target_size = 256 * 1024 * 1024 * 1024;
  cf_opt.cf_paths = {
    rocksdb::DbPath("/mnt/smartssd0/czn/raid_csd_test0",target_size),
    rocksdb::DbPath("/mnt/smartssd1/czn/raid_csd_test1",target_size),
    rocksdb::DbPath("/mnt/smartssd2/czn/raid_csd_test2",target_size),
    rocksdb::DbPath("/mnt/smartssd3/czn/raid_csd_test3",target_size),
  };
  std::vector<rocksdb::ColumnFamilyDescriptor> cf_descs = {
    rocksdb::ColumnFamilyDescriptor(rocksdb::kDefaultColumnFamilyName, cf_opt),
  };
