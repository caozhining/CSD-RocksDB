...
enum CompactionDevice : char {
  // The compaction is performed on the CPU by default
  kCompactionOnCPU = 0x0,
  // performed on CSD
  kCompactionOnCSD = 0x1,
};
...
enum CompactionCSDGenSSTfileSizePolicy : char {
  //Output file size limit = input file size average
  kCompactionCSDSSTavg = 0x0,
  //Output file size limit = 64MB
  KCompactionCSDSSTabove64 = 0x1,
  //Output file size limit = 64MB * layer
  KCompactionCSDSSTlayer = 0x2,
  //Output file size limie = approx(input size average, 64MB) to avoid small SST generation 
  kCompactionCSDSSTwtosmall = 0x3
};
...
struct AdvancedColumnFamilyOptions {
  ...
  // If compaction_device = kCompactionOnCSD, the compaction would calculate on CSD
  // Default: kCompactionOnCPU
  CompactionDevice compaction_device = kCompactionOnCPU;

  CompactionCSDPolicy compaction_csd_policy = kCompactionCSDArray;

  // If compaction_device = kCompactionOnCSD, CompactionKernelPath would store the address of kernel
  // Default: NULL
  std::string CompactionKernelPath; 

  // If compaction_device = kCompactionOnCSD, Compaction_accelerator_id would store the id of device
  // Default: 0
  std::vector<uint32_t> Compaction_accelerator_id;

  //compaction_csd_gen_sst_file_size_policy would determine sst file size generated by CSD
  CompactionCSDGenSSTfileSizePolicy compaction_csd_gen_sst_file_size_policy = kCompactionCSDSSTavg;
 
  // If compaction_device = kCompactionOnCSD, compaction_on_csd_threads will determine the number of kernel threads running on CSD.
  // Default: 1 
